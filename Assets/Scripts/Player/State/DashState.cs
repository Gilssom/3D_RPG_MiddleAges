...
namespace CharacterController
{
    public class DashState : BaseState
    {
        ...
        private void Dash()
        {
            Vector3 DashDir = (m_PlayerController.m_CulatedDirection == Vector3.zero) ? PlayerInfo.Instance.transform.forward : m_PlayerController.m_CulatedDirection;

            PlayerInfo.Instance.m_Anim.SetBool(Hash_IsDashBool, true);
            PlayerInfo.Instance.m_Anim.SetTrigger(Hash_DashTrigger);

            PlayerInfo.Instance.m_Rigid.velocity = DashDir * m_DashPower;
